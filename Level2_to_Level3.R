...
#### Output files:
# lvl3.tsv - mean cell counts, "GR values", and relative cell counts (averaged over technical replicates)
...
# Download Level 2 and Level 3 datasets
...
lvl2_gp = lvl2 %>% group_by(`Cell HMS LINCS ID`, `Cell Name`, `Small Molecule HMS LINCS ID`, `Small Molecule Name`, `Small Mol Concentration`, `Small Mol Conc Unit`, Replicate, `Seeding Density`) %>%
  ...
  dplyr::filter(`Small Mol Concentration` != 0) %>% dplyr::arrange(`Cell Name`, `Small Molecule Name`, Replicate, `Seeding Density`, `Small Mol Concentration`) %>% ungroup %>% as.data.frame()

write_tsv(lvl2_gp, "lvl3.tsv")
